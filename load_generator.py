...
def load(h_NHN, v, Theta_inf, S_w, A_he, Theta_b_0, R_th, Theta_surf_0, B, Phi, RR, m_Rw_0, m_Rs_0, ssb):  
    ...
    # Hilfsvariablen
    t_surf_calc = False  # "True" falls Oberlächentemp. bereits in diesem Modul ermittelt wird
    Theta_surf_sol = None
    ...
    if (m_Rs_0 > 0 or ssb is True):  # Schneedecke bildet sich
        sb_active = True  # snow-balancing aktivieren
    ...
    if (sb_active is True):  # "Schnee wird verzögert abgeschmolzen" (Bildung einer Schneedecke)
        ...
        if Q_0 < 0:  # keine nutzbare T-Differenz im Boden vorhanden (sibirische Verhältnisse)
            ...
            # Q_Verdunstung, Q_sen, Q_lat = 0
            Q_eva, Q_sen, Q_lat = 0, 0, 0
        ...
        # 2.2) Schmez- und Verlustleistungsterme (parametriert) - Parameter Q
        Q = sp.symbols('Q')  # thermische Leistung Q als Parameter definieren
        ...
        if Q_sol < 0:  # kein Wärmeentzug aus Erdboden
            ...
            # Q_Strahlung (identisch)

            # Q_Verdunstung (identisch)
    ...
    return Q_sol, t_surf_calc, Theta_surf_sol, m_Rw_1, m_Rs_1
